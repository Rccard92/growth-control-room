...
def test_synthesis_progress_range_in_batch_processor() -> None:
    ...
    assert "progress_percent=80" in source
    assert "progress_percent=95" in source
    assert "synthesize_batch" in source
    assert "apply_conflict_detection_to_batch" in source
    idx_conflict = source.index("apply_conflict_detection_to_batch")
    idx_synth = source.index("synthesize_batch")
    assert idx_conflict < idx_synth
