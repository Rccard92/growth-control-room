...
def test_faq_objections_proposal_parses_camelcase() -> None:
    proposal = BrandFaqObjectionsProposal.model_validate(
        {
            "generalFaq": [{"question": "Spedizioni?", "answer": "48h"}],
            "objections": ["Prezzo alto"],
            "recommendedAnswers": ["Valore artigianale"],
        }
    )
    assert proposal.general_faq is not None
    assert proposal.general_faq[0].question == "Spedizioni?"
...
def test_import_faq_objections_success_mock_ai() -> None:
    async def run() -> None:
        ...
        with (
            ...
            patch(
                ...
                new=AsyncMock(
                    return_value={
                        ...
                        "recommendedAnswers": ["Spiega qualità artigianale"],
                    }
                ),
            ),
        ):
            ...
            assert len(result.proposal.general_faq or []) == 1
            assert len(result.proposal.objections or []) == 1
