...
export function InventoryRiskPanel({ inventory }: InventoryRiskPanelProps) {
  const { inventorySummary, outOfStockProducts, lowStockProducts } = inventory;
  const urgent = [...outOfStockProducts, ...lowStockProducts].slice(0, 8);
        ...
        <ul className="shopify-inventory-list">
          {urgent.map((product) => (
            <li key={product.title} className="shopify-inventory-list__item">
              ...
              >
                {product.totalInventory === 0 ? "Out of stock" : `${product.totalInventory} unità`}
              </span>
            </li>
          ))}
        </ul>
